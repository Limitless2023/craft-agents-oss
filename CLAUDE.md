...
> **⚠️ Pi SDK 0.80.3 新增 jiti/static gotcha**：`pi-coding-agent@0.80.3` 精确依赖 **jiti 2.7.0**（暴露 `jiti/static` 子路径），但 hoist 的根 jiti 是 2.6.1（无 `./static`），`bun install` 只复用 hoist 版、**漏建嵌套 jiti 2.7.0** → `server:build:subprocess` 报 `Could not resolve "jiti/static"`。修复：`bun install --force` 补齐 `node_modules/@earendil-works/pi-coding-agent/node_modules/jiti@2.7.0` 后再 build subprocess。
>
